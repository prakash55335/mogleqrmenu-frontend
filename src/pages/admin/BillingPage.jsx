...
export default function BillingPage() {
  ...
  function openTaxEdit() {
    setTaxForm({ cgst: taxSettings.cgst, sgst: taxSettings.sgst, enabled: taxSettings.enabled })
    setShowTax(true)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#000', color: '#fff' }}>
      ...
      <div style={{ padding: '16px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {/* TAX SETTINGS BANNER */}
        <div style={{ backgroundColor: '#0d0d0d', border: '1px solid #27272a', borderRadius: '12px', padding: '14px 18px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '20px' }}>🧾</span>
            <div>
              <p style={{ color: '#a1a1aa', fontSize: '11px', margin: '0 0 2px', letterSpacing: '1px', textTransform: 'uppercase' }}>GST Settings</p>
              {taxSettings.enabled ? (
          ...
          <button onClick={openTaxEdit} style={{ background: 'transparent', color: '#facc15', border: '1px solid #facc15', borderRadius: '8px', padding: '8px 16px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
            ✏️ Edit Tax Settings
          </button>
        </div>
        ...
        <div style={{ marginBottom: '40px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
            <h2 style={{ color: '#4ade80', fontFamily: 'Bebas Neue,sans-serif', fontSize: 'clamp(20px,4vw,28px)', letterSpacing: '2px', margin: 0 }}>🟢 READY FOR BILLING</h2>
            <span style={{ backgroundColor: '#16a34a', color: '#fff', borderRadius: '20px', padding: '3px 12px', fontSize: '15px', fontWeight: '900' }}>{readyOrders.length}</span>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 260px), 1fr))', gap: '16px' }}>
              {readyOrders.map(order => {
                ...
                return (
                  <div key={order.id} style={{ backgroundColor: '#0d0d0d', border: '2px solid #16a34a', borderRadius: '14px', padding: '18px' }}
                    onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 20px rgba(22,163,74,0.2)'}
                    onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}
                  >
                    <h3 style={{ color: '#4ade80', fontSize: 'clamp(20px,4vw,26px)', fontWeight: '900', margin: '0 0 4px' }}>TABLE {order.table_number}</h3>
                    ...
                    <div style={{ backgroundColor: '#111', borderRadius: '10px', padding: '10px', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#71717a', marginBottom: '4px' }}>
                        <span>Subtotal</span><span>₹{parseFloat(order.total_amount).toFixed(2)}</span>
                      </div>
                      {taxSettings.enabled && (
                        <>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#71717a', marginBottom: '4px' }}>
                            <span>CGST ({taxSettings.cgst}%)</span><span>₹{tax.cgst.toFixed(2)}</span>
                          </div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#71717a', marginBottom: '6px' }}>
                            <span>SGST ({taxSettings.sgst}%)</span><span>₹{tax.sgst.toFixed(2)}</span>
                          </div>
                        </>
                      )}
                    ...
                    <button
                      ...
                      style={{ width: '100%', background: generating === order.id ? '#a16207' : 'linear-gradient(135deg,#facc15,#f59e0b)', color: '#000', border: 'none', borderRadius: '10px', padding: '13px', fontWeight: '900', fontSize: 'clamp(14px,2.5vw,16px)', cursor: generating === order.id ? 'not-allowed' : 'pointer' }}
                    >
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span style={{ backgroundColor: '#1a1a1a', border: '1px solid #3f3f46', color: '#a1a1aa', borderRadius: '20px', padding: '2px 10px', fontSize: '13px', fontWeight: '700' }}>{todayBills.length}</span>
            </div>
            {todayBills.length > 0 && (
          ...
          {todayBills.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {[...todayBills].reverse().map((bill, idx) => (
                <div key={bill.id || idx} style={{ backgroundColor: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '12px', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}
          ...
          <div style={{ backgroundColor: '#0d0d0d', border: '2px solid #facc15', borderRadius: '16px', width: '100%', maxWidth: '400px', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#111', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px' }}>
              <div>
                ...
                <p style={{ color: '#71717a', fontSize: '12px', margin: 0 }}>{taxForm.enabled ? 'GST applied on all bills' : 'No GST on bills'}</p>
              </div>
              <div onClick={() => setTaxForm(f => ({ ...f, enabled: !f.enabled }))}
            ...
            <input type="number" value={taxForm.cgst} onChange={e => setTaxForm(f => ({ ...f, cgst: e.target.value }))} placeholder="e.g. 2.5" min="0" max="50" step="0.5" disabled={!taxForm.enabled}
            ...
            <input type="number" value={taxForm.sgst} onChange={e => setTaxForm(f => ({ ...f, sgst: e.target.value }))} placeholder="e.g. 2.5" min="0" max="50" step="0.5" disabled={!taxForm.enabled}
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setShowTax(false)} style={{ flex: 1, backgroundColor: 'transparent', color: '#a1a1aa', border: '1px solid #3f3f46', borderRadius: '10px', padding: '12px', fontWeight: '700', cursor: 'pointer', fontSize: '14px' }}>Cancel</button>
              <button onClick={saveTax} style={{ flex: 2, background: 'linear-gradient(135deg,#facc15,#f59e0b)', color: '#000', border: 'none', borderRadius: '10px', padding: '12px', fontWeight: '900', cursor: 'pointer', fontSize: '14px' }}>Save Tax Settings</button>
            </div>
          </div>
        </div>
      )}
